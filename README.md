...
* `access-control-allow-origin`: Determines the value of the `Access-Control-Allow-Origin` HTTP response header to enable [CORS](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing). If not specified the header is not present in the responses.
