...
* `access-control-allow-origin`: Determines the presence and value of an `Access-Control-Allow-Origin` HTTP response header in order to enable [CORS](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing) (ff not specified the header is not present in the responses).
