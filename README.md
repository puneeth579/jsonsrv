...
* `access-control-allow-origin`: Determines the presence and value of a `Access-Control-Allow-Origin` HTTP response header to enable [CORS](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing). If not specified the header is not present in the responses.
